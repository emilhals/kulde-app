...
type HoveredItem = { id: string; anchor?: Placement }

type DragState =
  { active: false } | { active: true; startPos: Point; from: ItemAttachment }
...
export const Connector = ({
  ...
  const snapRef = useRef<{ position: Point; attachment: Attachment } | null>(
    null,
  )
  ...
  const detectConnection = (mousePosition: Point) => {
    ...
    while (node) {
      ...
      if (proxyObj?.type === 'connections' || proxyObj?.type === 'items') {
        return proxyObj
      }
      node = node.getParent()
    }
  ...
  const detectConnectable = (
    ...
    if (hoveredObject.type === 'connections') {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        const point = getClosestPointOnSegment(stagePointer, seg.start, seg.end)
      ...
      if (closest) {
        ...
        return { attachment: currentTarget, position: closest.position }
      }
    }
  ...
  const handleAnchorDragStart = (e: KonvaEventObject<DragEvent>) => {
    ...
    dragState.current = {
      ...
      from: {
        ...
        placement: placement,
        t: computeT(placement, { x: anchorPos.x, y: anchorPos.y }, item),
      },
    }
  }
  ...
  const handleAnchorDragMove = (e: KonvaEventObject<DragEvent>) => {
    ...
    if (hoveredObject?.type === 'items') {
      ...
      setHoveredItem({ id: hoveredObject.id, anchor })
    } else {
      setHoveredItem(null)
    }

    const targetPosition = snap ? snap.position : pointerStage
    const to: Attachment = { type: 'free', position: targetPosition }
  ...
  const handleAnchorDragEnd = (e: KonvaEventObject<DragEvent>) => {
    ...
    if (dragState.current.active && hoveredItem) {
      const from = stage.findOne('#' + dragState.current.from.itemId)
      const to = stage.findOne('#' + hoveredItem.id)

      if (!from || !to) return

      from.setAttr(
        'connection-side',
        dragState.current.from.placement.toLowerCase(),
      )
      to.setAttr('connection-side', hoveredItem.anchor?.toLowerCase())
    }
    ...
    if (to.type === 'free' || (to.type === 'item' && from.itemId === to.itemId))
      return

    snapRef.current = null
    dragState.current = { active: false }
    const connection: WithoutId<ConnectionData> = {
      type: 'connections',
