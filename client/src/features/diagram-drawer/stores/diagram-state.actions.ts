...
export const addToStore = (
  property: WithoutId<Item> | WithoutId<Text> | WithoutId<ConnectionData>,
) => {
  const id = uuidv4()
  ...
  if (proxy.type === 'items') {
    ...
    const filteredConnections = connections.filter(
      (connection) =>
        ...
        (connection.to.type === 'item' && connection.to.itemId === proxy.id),
    )
    ...
    const attachedTexts = texts.filter(
      (text) => text.anchor?.type === 'item' && text.anchor.itemId === proxy.id,
    )
