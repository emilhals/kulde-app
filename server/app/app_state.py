...
class AppState:
    ...
    def __post_init__(self) -> None:
        ...
        self.compressor = Compressor(run_state=RunStateEnum['IDLE'], power_state=PowerStateEnum['ON'])
    
    async def run_simulation(self) -> None:
        await self.compressor.start(controller=self.controller, room=self.room)
        await self.room.start(controller_setpoint=self.controller.set_point, compressor_run_state=self.compressor.run_state)

    async def get_values(self) -> dict[str, int | RunStateEnum | PowerStateEnum | dict[str, float | int]]:
        return {
    ...
    async def update_controller_params(self, set_point) -> None:
        logger.debug("setpoint: %s", set_point)
